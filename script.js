...
function sub_topics() {
    ...
    var stopic = 'outPeloporTest'; //publish
...
function send_message(command, aksi) {
    ...
    var topic = 'inPeloporTest'; //subscribe
    message = new Paho.MQTT.Message(s_msg);
    if (topic == "")
        message.destinationName = "inPeloporTest";
    else
...
function ledState(state) {
    ...
    message.destinationName = "inPeloporTest"; //subscribe
    mqtt.send(message);
}
